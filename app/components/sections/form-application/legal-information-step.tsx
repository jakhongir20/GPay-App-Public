"use client";

import React, { FC, useEffect } from "react";
import { Controller, useForm } from "react-hook-form";
import { useParams, useRouter } from "next/navigation";
import { Checkbox } from "@/app/components/shared/checkbox";
import { FormInput } from "@/app/components/shared/form";
import { useFormStore } from "@/app/store/form-store";
import { cn } from "@/app/shared/helpers/helper-functions";

interface Props {
  className?: string;
}

interface FormData {
  brandName: string;
  legalEntityName: string;
  directorFullName: string;
  oked: string;
  soato: string;
  legalAddress: string;
  isBrandNameMatchesLegalEntity: boolean;
}

export const LegalInformationStep: FC<Props> = ({ className }) => {
  const params = useParams();
  const router = useRouter();
  const lang = (params?.lang as string) || "ru";
  const { formData, setFormData } = useFormStore();

  const {
    register,
    handleSubmit,
    watch,
    control,
    setValue,
    getValues,
    formState: { errors },
  } = useForm<FormData>({
    mode: "onBlur",
    defaultValues: {
      brandName: formData.brandName || "",
      legalEntityName: formData.legalEntityName || "",
      directorFullName: formData.directorFullName || "",
      oked: formData.oked || "",
      soato: formData.soato || "",
      legalAddress: formData.legalAddress || "",
      isBrandNameMatchesLegalEntity: formData.isBrandNameMatchesLegalEntity ?? false,
    },
  });

  const isBrandNameMatchesLegalEntity = watch("isBrandNameMatchesLegalEntity");
  const legalEntityNameValue = watch("legalEntityName");

  // Sync brand name when checkbox is checked
  useEffect(() => {
    if (isBrandNameMatchesLegalEntity && legalEntityNameValue) {
      setValue("brandName", legalEntityNameValue, {
        shouldValidate: true,
        shouldDirty: true,
      });
    }
  }, [isBrandNameMatchesLegalEntity, legalEntityNameValue, setValue]);

  const onSubmit = (data: FormData) => {
    // Save form data to store (merge with existing data)
    setFormData({
      ...formData,
      ...data,
    });

    // Navigate to next page after successful submission
    router.push(`/${lang}/form-slug-3`);
  };

  return (
    <section className={cn(className)}>
      <div className="flex flex-col">
        <h2 className="text-section-title mb-4 lg:mb-6 max-w-[600px]">
          Юридическая информация
        </h2>
        <p className="text-section-subtitle">
          Заполните ваши юридические данные
        </p>
      </div>
      <hr className={"border-t w-full border-[#1C1C1C] my-8"} />
      <div className={"p-6 bg-[#141414] rounded-xl"}>
        <div className={"flex sm:flex-row flex-col sm:items-center justify-between gap-2 mb-8"}>
          <h4 className={"text-xl text-text-primary "}>Юридическая информация</h4>
          <div className={"flex gap-3 items-center"}>
            <div className={"rounded-[40px] bg-[rgba(138,225,106,0.10)] min-w-6 h-6 flex items-center justify-center"}>
              <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
                <path d="M3 8.5L7 12.5L13 3.5" stroke="#8AE16A" strokeWidth="1.5" strokeLinecap="round"
                      strokeLinejoin="round" />
              </svg>
            </div>
            <span className={"text-[#8AE16A]"}>Заполнено автоматически</span>
          </div>
        </div>
        <form onSubmit={handleSubmit(onSubmit)} className="w-full space-y-6">
          <div>
            <FormInput
              label="Брендовое название"
              required
              type="text"
              placeholder="Например: «Global Pay»"
              disabled={isBrandNameMatchesLegalEntity}
              register={register("brandName", {
                required: "Пожалуйста, введите брендовое название",
                minLength: {
                  value: 2,
                  message: "Название должно содержать минимум 2 символа",
                },
                maxLength: {
                  value: 120,
                  message: "Название должно содержать не более 120 символов",
                },
                setValueAs: (value: string) => value?.trim() ?? "",
              })}
              error={errors.brandName}
            />
            <Controller
              name="isBrandNameMatchesLegalEntity"
              control={control}
              render={({ field: { value, onChange } }) => (
                <Checkbox
                  id="legalName"
                  className={"mt-4"}
                  label={"Совпадает с юридическим названием"}
                  checked={value}
                  onChange={(event) => {
                    const checked = event.target.checked;
                    onChange(checked);
                    if (checked) {
                      const legalName = getValues("legalEntityName");
                      setValue("brandName", legalName, { shouldValidate: true, shouldDirty: true });
                    }
                  }}
                />
              )}
            />
          </div>

          <FormInput
            label="Полное наименование юр. лица"
            required
            type="text"
            placeholder="Например: ООО «Global Pay»"
            register={register("legalEntityName", {
              required: "Пожалуйста, введите полное наименование",
              minLength: {
                value: 2,
                message: "Минимум 2 символа",
              },
              maxLength: {
                value: 200,
                message: "Максимум 200 символов",
              },
              setValueAs: (value: string) => value?.trim() ?? "",
            })}
            error={errors.legalEntityName}
          />

          <FormInput
            label="ФИО Директора"
            required
            type="text"
            placeholder="Введите ФИО директора"
            register={register("directorFullName", {
              required: "Пожалуйста, введите ФИО директора",
              minLength: {
                value: 5,
                message: "ФИО должно содержать минимум 5 символов",
              },
              setValueAs: (value: string) => value?.trim() ?? "",
            })}
            error={errors.directorFullName}
          />

          <div>
            <label className="mb-2 block text-base tracking-[-0.16px] text-white/60">
              <div className={"flex gap-1"}>
                ОКЭД <span className="text-text-primary">*</span>
                <p className={"mt-1 ml-1"}>
                  <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
                    <path fillRule="evenodd" clipRule="evenodd"
                          d="M15 8C15 11.866 11.866 15 8 15C4.13401 15 1 11.866 1 8C1 4.13401 4.13401 1 8 1C11.866 1 15 4.13401 15 8ZM9 11.5C9 12.0523 8.55228 12.5 8 12.5C7.44772 12.5 7 12.0523 7 11.5C7 10.9477 7.44772 10.5 8 10.5C8.55228 10.5 9 10.9477 9 11.5ZM7.29303 5.29289C7.68355 4.90237 8.31671 4.90237 8.70724 5.29289C9.09776 5.68342 9.09776 6.31658 8.70724 6.70711C8.58354 6.83081 8.43743 6.91429 8.2827 6.95959C7.82404 7.09387 7.25013 7.52576 7.25013 8.25V8.50022C7.25013 8.91444 7.58592 9.25022 8.00013 9.25022C8.41435 9.25022 8.75013 8.91444 8.75013 8.50022V8.38522C9.12313 8.26818 9.47391 8.06176 9.7679 7.76777C10.7442 6.79146 10.7442 5.20854 9.7679 4.23223C8.79159 3.25592 7.20868 3.25592 6.23237 4.23223C5.93947 4.52513 5.93947 5 6.23237 5.29289C6.52526 5.58579 7.00013 5.58579 7.29303 5.29289Z"
                          fill="#EDEDED" fillOpacity="0.6" />
                  </svg>
                </p>
              </div>
            </label>
            <div className="relative">
              <input
                {...register("oked", {
                  required: "Пожалуйста, укажите ОКЭД",
                  setValueAs: (value: string) => value?.replace(/\s+/g, "") ?? "",
                })}
                type="text"
                placeholder="Введите ОКЭД"
                className={cn(
                  "w-full appearance-none rounded-lg bg-[#212121] px-4 py-4 pr-10 text-white/60 outline-none transition-colors border border-transparent",
                  errors.oked
                    ? "border-red-500 focus:border-red-500 focus:ring-1 focus:ring-red-500"
                    : "hover:border-gray-500 focus:border-button-primary focus:ring-1 focus:ring-button-primary",
                )}
              />
            </div>
            {errors.oked && (
              <p className="mt-1 text-sm text-red-500">{errors.oked.message}</p>
            )}
          </div>

          <div>
            <label className="mb-2 block text-base tracking-[-0.16px] text-white/60">
              <div className={"flex gap-1"}>
                Регион/район (SOATO) <span className="text-text-primary">*</span>
                <p className={"mt-1 ml-1"}>
                  <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
                    <path fillRule="evenodd" clipRule="evenodd"
                          d="M15 8C15 11.866 11.866 15 8 15C4.13401 15 1 11.866 1 8C1 4.13401 4.13401 1 8 1C11.866 1 15 4.13401 15 8ZM9 11.5C9 12.0523 8.55228 12.5 8 12.5C7.44772 12.5 7 12.0523 7 11.5C7 10.9477 7.44772 10.5 8 10.5C8.55228 10.5 9 10.9477 9 11.5ZM7.29303 5.29289C7.68355 4.90237 8.31671 4.90237 8.70724 5.29289C9.09776 5.68342 9.09776 6.31658 8.70724 6.70711C8.58354 6.83081 8.43743 6.91429 8.2827 6.95959C7.82404 7.09387 7.25013 7.52576 7.25013 8.25V8.50022C7.25013 8.91444 7.58592 9.25022 8.00013 9.25022C8.41435 9.25022 8.75013 8.91444 8.75013 8.50022V8.38522C9.12313 8.26818 9.47391 8.06176 9.7679 7.76777C10.7442 6.79146 10.7442 5.20854 9.7679 4.23223C8.79159 3.25592 7.20868 3.25592 6.23237 4.23223C5.93947 4.52513 5.93947 5 6.23237 5.29289C6.52526 5.58579 7.00013 5.58579 7.29303 5.29289Z"
                          fill="#EDEDED" fillOpacity="0.6" />
                  </svg>
                </p>
              </div>
            </label>
            <div className="relative">
              <input
                {...register("soato", {
                  required: "Пожалуйста, укажите регион/район",
                })}
                type="text"
                placeholder="Введите регион/район"
                className={cn(
                  "w-full appearance-none rounded-lg bg-[#212121] px-4 py-4 pr-10 text-white/60 outline-none transition-colors border border-transparent",
                  errors.soato
                    ? "border-red-500 focus:border-red-500 focus:ring-1 focus:ring-red-500"
                    : "hover:border-gray-500 focus:border-button-primary focus:ring-1 focus:ring-button-primary",
                )}
              />
            </div>
            {errors.soato && (
              <p className="mt-1 text-sm text-red-500">{errors.soato.message}</p>
            )}
          </div>

          <div>
            <FormInput
              label="Юридический адрес"
              required
              type="text"
              placeholder="Введите юридический адрес"
              register={register("legalAddress", {
                required: "Пожалуйста, введите юридический адрес",
                minLength: {
                  value: 5,
                  message: "Адрес должен содержать минимум 5 символов",
                },
                setValueAs: (value: string) => value?.trim() ?? "",
              })}
              error={errors.legalAddress}
            />
            <small className={"text-[rgba(237,237,237,0.60)] text-xs"}>
              Укажите адрес регистрации компании, как в учредительных документах
            </small>
          </div>
          {/*  form action*/}
          <div className={"py-6 flex items-center justify-between"}>
            <div className="flex items-center justify-center gap-2">
              <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
                <path fillRule="evenodd" clipRule="evenodd"
                      d="M15 8C15 8.98224 14.5279 9.85423 13.7983 10.4015C13.9273 11.3044 13.6446 12.2549 12.95 12.9494C12.2553 13.6441 11.3048 13.9269 10.4019 13.7978C9.85465 14.5277 8.98248 15 8 15C7.01773 15 6.14572 14.5279 5.59848 13.7983C4.69556 13.9273 3.74514 13.6445 3.05055 12.9499C2.35595 12.2553 2.07316 11.3048 2.2022 10.4019C1.47228 9.85464 1 8.98248 1 8C1 7.01773 1.47208 6.14573 2.20173 5.59848C2.07273 4.69557 2.35552 3.74513 3.0501 3.05055C3.74471 2.35595 4.69519 2.07316 5.59813 2.2022C6.14535 1.47228 7.01752 1 8 1C8.98224 1 9.85423 1.47205 10.4015 2.20167C11.3044 2.07265 12.2549 2.35544 12.9494 3.05003C13.6441 3.74465 13.9269 4.69516 13.7978 5.59813C14.5277 6.14535 15 7.01752 15 8ZM11.7086 5.15657C12.0364 5.40984 12.0967 5.88086 11.8435 6.20862L7.59347 11.7086C7.45747 11.8846 7.25037 11.9912 7.0281 11.9995C6.80583 12.0078 6.59133 11.9171 6.44254 11.7518L4.19254 9.25176C3.91544 8.94388 3.9404 8.46966 4.24828 8.19257C4.55617 7.91547 5.03038 7.94043 5.30748 8.24831L6.95616 10.0802L10.6565 5.29145C10.9098 4.96369 11.3808 4.9033 11.7086 5.15657Z"
                      fill="#EDEDED" />
              </svg>
              <span
                className="font-helvetica-neue-cyr text-sm font-normal text-[rgba(237,237,237,0.40)]"
              >Сохранено в 10:21</span
              >
            </div>
            <div className={"flex gap-3"}>
              <button
                type={"button"}
                className="h-12 px-4 sm:px-6  btn-user rounded-lg"
                onClick={() => router.push(`/${lang}/form-slug-1`)}
              >
                Назад
              </button>
              <button
                type="submit"
                className="h-12 px-4 sm:px-6 rounded-lg bg-button-primary text-base font-medium text-white transition-colors"
              >
                Далее
              </button>
            </div>
          </div>
        </form>
      </div>
    </section>
  );
};
